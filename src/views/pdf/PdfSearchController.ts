...
export class PdfSearchController {
	...
	open(): void {
		...
		this.searchInputEl.addEventListener('keydown', (e) => {
			...
			if (e.key === 'Enter') {
				e.preventDefault();
				e.shiftKey ? this.goToMatch(this.currentIdx - 1) : this.goToMatch(this.currentIdx + 1);
		...
		this.searchInputEl.addEventListener('input', () => {
			...
			this.debounceTimer = setTimeout(() => this.performSearch(this.searchInputEl!.value), 300);
		});
	...
	get hasMatches(): boolean { return this.matches.length > 0; }
	...
	private goToMatch(idx: number): void {
		...
		this.currentIdx = ((idx % this.matches.length) + this.matches.length) % this.matches.length;
